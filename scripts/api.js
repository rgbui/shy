...
push('/datastore/stat/fields', '{groupFilters?: { id: string, filter: Record<string, any> }[],ws:LinkWs,wsId?:string,schemaId:string,stats:{fieldId:string,stat:string}[],filter?:Record<string,any>}', '{ok:boolean,data:{stats?:{fieldId:string,stat:string,value:number,total?:number}[],groupStats?:({fieldId:string,stat:string,value:number,total?:number}[])[]},warn:string}', ['get']);
push('/datastore/board/statistics','{ws:LinkWs,wsId?:string,schemaId:string,group?:string,page?:number,size?:number,filter?:Record<string, any>,sorts?:Record<string, 1|-1>,hideGroups?:string[],isIgnoreEmpty?:boolean,isStatTotal?: boolean,}', '{ok:boolean,data:{list:{id:string,list:any[],total?:number}[]},warn:string}', ['get']);
push('/datastore/board/stat/fields','{groupFilters?: { id: string, filter: Record<string, any> }[],ws:LinkWs,wsId?:string,schemaId:string,group?:string,stat:{fieldId:string,stat:string},filter?:Record<string, any>,hideGroups?:string[],isIgnoreEmpty?:boolean}','SockResponse<{stats?:{id:string,value:number|Date,total?:number|Date}[],groupStats?:({fieldId:string,stat:string,value:number,total?:number}[])[]}>',['get']);
...
push('/ws/create', '{text:string,serviceAddress?:string,datasource?: "private-cloud" | "public-cloud" | "private-local",datasourceClientId?:string,templateUrl?:string}', 'SockResponse<{workspace:Record<string,any>,pids:any[]}>', ['put'])
...
push('/ws/channel/send', '{ sockId?: string,wsId?: string,roomId: string,content?: string,replyId?: string, files?:any[],refs?:any[],mentions?:string[],robotId?:string,isRobotSend?: boolean,newLine?: boolean}', 'SockResponse<{id:string,seq:number,createDate:Date}>', ['put'])
...
push('/ws/user/basic', '{wsId:string,userid:string}', 'SockResponse<{wsMember:WorkspaceMember}>', ['get']);
