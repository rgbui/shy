    ...
    return <div>
        ...
        <div className="flex-center-full r-padding-30 r-round-8 r-bg-white gap-t-20">
            <div className="gap-r-20 w33 r-gap-b-10 shy-site-block-card" >
                <div className=" shy-site-block-head flex r-gap-r-5" style={{ fontSize: 28 }}><img style={{ height: config.isUS ? 28 * 2 : undefined }} src={UrlRoute.getUrl('/static/img/local.svg')} /><span><S>本地及私有化免费</S></span></div>
                <div className=" f-14 text-1 l-24 " style={{ height: config.isUS ? 100 : 'auto' }}><Sp text='适用于本地数据安全敏感的用户'>适用于本地数据安全、敏感的用户。<br />支持自搭服务器，数据自主管理。</Sp></div>
                <div className=" bold" style={{ fontSize: 20 }}><S>信任</S></div>
                ...
                <div className=" f-14 l-24 text-1 flex-top"> <span className="flex-center size-24 flex-fixed gap-r-5"><Icon icon={CheckSvg} size={12}></Icon></span><S text='我们想让更多的人用上诗云仅此而已'>我们想让更多的人用上诗云，仅此而已</S></div>
                <div className=" flex gap-t-20"><a className="padding-w-14 padding-h-5 round-8 cursor text-white" style={{ background: '#0BDCC0' }} href={UrlRoute.getUrl('/price')}><S>了解详情</S></a></div>
            </div>
            <div className="gap-l-20 w33 r-gap-b-10 shy-site-block-card">
                <div className=" shy-site-block-head flex  r-gap-r-5" style={{ fontSize: 28 }} ><img style={{ height: config.isUS ? 28 * 2 : undefined }} src={UrlRoute.getUrl('/static/img/online.svg')} /><span><S>云端按量计费</S></span></div>
                <div className=" f-14 text-1 l-24 " style={{ height: config.isUS ? 100 : 'auto' }}><Sp text='适用于云端协作办公'>适用于云端协作办公。<br />为个人及团队的生产效率、社区协作而买单。</Sp></div>
                <div className=" bold" style={{ fontSize: 20 }}><S>承诺</S></div>
                ...
                <div className=" flex  gap-t-20"><a className="padding-w-14 padding-h-5 round-8 cursor text-white bg-primary bg-primary-hover" href={UrlRoute.getUrl('/price')}><S>了解详情</S></a></div>
            </div>
        </div>
    </div>
}
...
export function PricingPackage(props: { wrap?: boolean }) {
    ...
    var pres = <>
        <div className="gap-10 padding-14 shy-site-block-card">
            ...
            <div className="bg-primary bg-primary-hover gap-h-20 text-white flex-center h-50 round cursor"><S>立即充值</S></div>
        ...
        <div className="gap-10  padding-14 shy-site-block-card">
            ...
            <div className="bg-primary bg-primary-hover  gap-h-20  text-white flex-center h-50 round cursor"><S>升级购买</S></div>
    ...
    var lasts = <>
        <div className="gap-10 padding-14 shy-site-block-card">
            ...
            <div className="bg-primary-1  bg-primary-1-hover  gap-h-20  text-white flex-center h-50 round cursor"><S>升级购买</S></div>
    ...
    return <div>
        ...
        {!props.wrap && <div className="flex gap-t-20 r-w25"> {pres}{lasts}</div>}
    </div>
