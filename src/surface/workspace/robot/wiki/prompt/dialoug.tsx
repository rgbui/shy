...
export class RobotInfoPromptForm extends EventsComponent {
    ...
    render(): ReactNode {
        return <div className="w-450 padding-14 round ">
            ...
            <div>
                {this.prompt.apply == RobotApply.channel && <div className="remark f-14 gap-b-10"><S>模板参数</S>{lst('{prompt}:提示，{context}:上下文')}</div>}
                ...
                {this.prompt.apply == RobotApply.askWrite && <div className="remark f-14 gap-b-10"><S>模板参数</S>{lst('{prompt}:提示，{context}:上下文')}</div>}
                {this.prompt.apply == RobotApply.selectionAskWrite && <div className="remark f-14 gap-b-10"><S>模板参数</S>{lst('{selection}:选中内容，{prompt}:提示，{context}:上下文')}</div>}
            </div>
            <Divider></Divider>
