...
import { CheckSvg, DotsSvg, Edit1Svg, EditSvg, NoneSvg, PlusSvg, TrashSvg } from "rich/component/svgs";
...
export class RobotInfoPromptView extends React.Component<{ robot: RobotInfo }>{
    ...
    render() {
        ...
        return <div>
            <div className="h3 flex"><span className="flex-auto">Prompt模板</span><span onClick={e => this.addPrompt()} className=" flex-fixed size-24 item-hover round cursor flex-center"><Icon icon={PlusSvg}></Icon></span></div>
            <div className="flex flex-wrap">
                {(robot.prompts || []).map(pro => {
                    var ra = RobotApplyOptions.find(g => g.value == pro.apply);
                    if (ra?.value == RobotApply.none) ra = null;
                    return <div className={"card flex-fixed w-250 h-120 round padding-10" + (pro.abled == false ? " op-4" : "")} key={pro.id}>
                        <div className="flex h-30">
                            <span className="flex-fixed size-24 round item-hover cursor"><Icon size={20} icon={pro.icon || Edit1Svg}></Icon></span>
